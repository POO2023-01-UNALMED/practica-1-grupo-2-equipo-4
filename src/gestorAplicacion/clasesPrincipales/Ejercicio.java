...
public class Ejercicio extends Movimiento /*implements Serializable*/ {
    ...
    public static Ejercicio generarEjercicio(Movimiento movimiento, ObjetivoCliente objetivo, int intensidad) {
        int repeticiones = 0;
        int series = 0;
        ...
        if (movimiento.getTipoEjercicio() == TipoEjercicio.CALENTAMIENTO){
            ...
            switch(objetivo){
                case AUMENTAR:
                    repeticiones = (8 + random.nextInt(4)) * intensidad;
                    series = (1 + random.nextInt(2)) * intensidad;
                    break;
                case DEFINIR:
                    repeticiones = (14 + random.nextInt(3)) * intensidad;
                    series = (3 + random.nextInt(2)) * intensidad;
                    break;
                case BAJARPESO:
                    repeticiones = (9 + random.nextInt(5)) * intensidad;
                    series = (2 + random.nextInt(2)) * intensidad;
                    break;
                case MANTENER:
                    repeticiones = (11 + random.nextInt(2)) * intensidad;
                    series = (2 + random.nextInt(2)) * intensidad;
                    break;
                case ACONDICIONAR:
                    repeticiones = (7 + random.nextInt(3)) * intensidad;
                    series = (4 + random.nextInt(2)) * intensidad;
                    break;
            }
        }
